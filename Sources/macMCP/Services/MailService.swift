    ...
    static func register(_ registry: ToolRegistry) {
        ...
        registry.register(
            MCPTool(
                ...
                description: "List the mail accounts configured in Mail.app. Returns a JSON array of account names, each usable as the account argument of every other mail tool. Mail's local mailboxes are not an account and are not listed here; they are addressed as \"On My Mac\" (see mail_list_mailboxes). Under a resource scope that names mail accounts, only those accounts are returned",
        ...
        registry.register(
            MCPTool(
                ...
                inputSchema: schema(
                    properties: [
                        "query": stringProp("Literal text matched case-insensitively as a substring of the subject or the sender (and of recipients or body when those options are on). No operators, wildcards, field prefixes or date filters: the whole string is one substring"),
