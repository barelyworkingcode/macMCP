...
- **Mail reads must use bulk column fetches.** `mbox.messages.subject()` costs one Apple Event per column per mailbox regardless of message count (~0.07ms/message; 14k subjects in ~1s). Three things are forbidden in a read path, all measured against a 14,004-message mailbox:
  ...
  **A scan that read nothing is an error, not an empty result.** `unstable_mailboxes: ["Alice:INBOX"]` beside `total_messages: 0` and `isError: false` is an affirmative claim that a mailbox holding thousands of messages is empty, and a caller who does not know to read that field cannot tell the two apart. When every mailbox in scope came back unstable the call now refuses and says so; a partial read still returns its rows, with `scan_complete` (reported unconditionally, true or false) and a `note` saying the counts are a floor. `scan_complete` covers `skipped_mailboxes` and `failed_accounts` too, which are short in exactly the same way.
...
- **A JXA specifier is not the object it resolves to, and a positional one is a bug.** `mail.accounts()`, `acct.mailboxes()` and `mbox.messages()` all hand back specifiers indexed by **position**, and JXA re-evaluates a specifier on *every* property access rather than snapshotting. `found = mbox.messages[k]` therefore means "whatever is at position k right now": bind it, let one message leave the mailbox, and the same variable answers for a different message — no error, no warning. Measured on the fixture at ~3,000 messages under continuous delivery, `mail_get_email` was wrong in **26 of 60** calls, and in 18 of those it returned the right id, the right subject and the right `rfc_message_id` beside **another message's body** — a shape nothing in the response lets a caller detect. The exposure is the whole call, not a gap between two reads. Four places had it (#50): `findMessageJXA`, the body-scan pass, `mail_create_draft`'s find-back, and the mailbox collection itself.
  ...
  **`byName` is not a general answer for mailboxes.** Mail flattens an account's mailbox tree and reports leaf names, so `acct.mailboxes.name()` can return `["Archive","Projects","Sub","Archive",…]` — two different mailboxes called `Archive` (top-level and `Projects/Archive`), and a `Sub` (`Archive/Sub`) that `mailboxes.byName('Sub')` **cannot reach**: the specifier's `exists()` is false while position 2 reads fine. So `boundByNameJXA` takes the names from one bulk `collection.name()`, binds by name only when the name is unique *and* `exists()`, and otherwise keeps the positional specifier. Skipping either check silently drops a mailbox from every scan or merges two into one.
